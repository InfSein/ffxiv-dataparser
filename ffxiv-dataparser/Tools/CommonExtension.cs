...
internal static class CommonExtension
{
    ...
    /// 以较高的质量将图像存储到本地
    /// </summary>
    public static void HQSave(this Image image, string path)
    {
        EncoderParameters encoderParams = new();
        encoderParams.Param[0] = new(ImagingEncoder.Quality, 100);
        var codecInfo = ImageCodecInfo.GetImageEncoders()
            .FirstOrDefault(ici => ici.MimeType == "image/png")
            ?? throw new Exception("ImageCodecInfo.GetImageEncoders got a null");
